...
public:
    ...
    class AutomataDescription {
        unsigned int id;
        std::string name;
        dim dimension;
    public:
        AutomataDescription(unsigned int i, std::string n, dim d);
...
// Automata management :
    /* Return a vector of AutomataDescription, each one containing
     * the id, the name and the dimension of the automaton */
     std::vector<const AutomataManager::AutomataDescription> getArrayOfAutomata() const;
     ...
     * indicated by the id i of the automaton */
     Automaton * selectedAutomata(unsigned int const i);
